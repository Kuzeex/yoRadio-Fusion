...
struct config_t
{
  ...
  uint8_t   vuBarGapStr;       // 0..6
  uint8_t   vuBarCountBbx;     // 8..64
  uint8_t   vuBarGapBbx;       // 0..6
  uint8_t   vuBarCountStd;     // 8..64
  uint8_t   vuBarGapStd;       // 0..6
  uint8_t   vuFadeSpeedDef;    // 0..10
  uint8_t   vuFadeSpeedStr;    // 0..10
  uint8_t   vuFadeSpeedBbx;    // 0..10
  uint8_t   vuFadeSpeedStd;    // 0..10
  uint8_t   vuBarHeightDef, vuBarHeightStr, vuBarHeightBbx, vuBarHeightStd;   // 5..50
  uint8_t   vuAlphaUpDef,  vuAlphaUpStr,  vuAlphaUpBbx,  vuAlphaUpStd;   // 0..100
  uint8_t   vuAlphaDnDef,  vuAlphaDnStr,  vuAlphaDnBbx,  vuAlphaDnStd;   // 0..100
  uint8_t   vuPeakUpDef,   vuPeakUpStr,   vuPeakUpBbx,   vuPeakUpStd;    // 0..100
  uint8_t   vuPeakDnDef,   vuPeakDnStr,   vuPeakDnBbx,   vuPeakDnStd;    // 0..100
  uint8_t   vuExpoDef, vuExpoStr,  vuExpoBbx,  vuExpoStd;   // expo = value/100.0f
  uint8_t   vuFloorDef, vuFloorStr, vuFloorBbx, vuFloorStd;  // % alsó küszöb (zajkapu)
  uint8_t   vuCeilDef, vuCeilStr, vuCeilBbx, vuCeilStd;   // % felső határ (headroom)
  uint8_t   vuGainDef, vuGainStr, vuGainBbx, vuGainStd;   // szorzó = value/100.0f
  uint8_t   vuKneeDef, vuKneeStr, vuKneeBbx, vuKneeStd;   // % “puha” küszöb szélesség
  uint8_t   vuMidPctDef;   // default 60  (0.6)
  uint8_t   vuMidPctStr;   // default 60  (0.6)
  uint8_t   vuMidPctBbx;   // default 60  (0.6)
  uint8_t   vuMidPctStd;   // default 60  (0.6)
  uint8_t   vuHighPctDef;  // default 80  (0.8)
  uint8_t   vuHighPctStr;  // default 80  (0.8)
  uint8_t   vuHighPctBbx;  // default 80  (0.8)
  uint8_t   vuHighPctStd;  // default 80  (0.8)
  uint16_t  vuPeakColor;  // RGB565 (pl. 0xFFFF)
  uint16_t  vuMidColor;
  uint8_t   vuMidOn; 
  uint8_t   vuPeakOn; 
  uint16_t  vuMidUserColor;
  uint16_t  vuPeakUserColor;
  uint8_t   dateFormat;   // 0..5
  uint8_t   showNameday;
  uint8_t   clockFontId;
...
/* ----- Auto On-Off Timer --original in private class--- */
  ...
  public:
    ...
    void init();
    void loadTheme();
    void loadThemeFromHFile();  /* ------------------myTheme webUI upload---------------*/
    void applyThemeColor(const String& name, uint16_t color); /* ------------------myTheme webUI upload---------------*/
...
#define REF_BY_LAYOUT(store, field, ly) \
  ((ly)==1 ? (store).field##Str : (ly)==2 ? (store).field##Bbx : (ly)==3 ? (store).field##Std : (store).field##Def)
