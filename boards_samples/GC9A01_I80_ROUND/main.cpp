...
#include <Wire.h>
#include "core/touchscreen_cst816.h"
#ifdef USE_NEXTION
...
void setup() {
  ...
  //
  // 1) I2C1 indulása — CSAK ITT!
  //
  TouchWire.begin(TS_SDA, TS_SCL, 100000);
  TouchWire.setClock(100000);
  Serial.println("[OK] TouchWire.begin()");

  //
  // 2) CST816 driver indulása
  //
  ts.begin(240, 240, config.store.fliptouch);
  Serial.println("[OK] ts.begin()");
